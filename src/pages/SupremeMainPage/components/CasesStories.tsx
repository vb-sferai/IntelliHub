...
const parseNumericValue = (
  ...
  // Проверяем есть ли пробелы между цифрами (для форматирования 11 000)
  const hasSpace = /\d\s+\d/.test(value);
  // Убираем только пробелы МЕЖДУ цифрами (11 000 → 11000), но сохраняем пробелы перед единицами
  const cleanValue = value.replace(/(\d)\s+(?=\d)/g, '$1');
